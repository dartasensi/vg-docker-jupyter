...
# example: install_nodejs <node_version>
function install_nodejs () {
  ...
  if [[ -n "${USER_NODE_VERSION}" ]] ; then
    # NodeSource repo configuration
    GPG_URL='https://deb.nodesource.com/gpgkey/nodesource.gpg.key'
    GPG_KEYRING_FILENAME='nodesource-keyring.gpg'
    APT_REPO_DEB_TYPE='deb'
    APT_REPO_DEB_URL="https://deb.nodesource.com/${USER_NODE_VERSION}"
    APT_REPO_DEB_DISTRO=$(lsb_release -cs)
    APT_REPO_DEB_CHANNEL='main'
    APT_REPO_FILENAME='nodesource.list'
